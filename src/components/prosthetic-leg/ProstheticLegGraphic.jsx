...
const ProstheticLegGraphic = ({ scale = 0.50, selectedPart }) => {
        ...
        >
          ...
          <div className="leg-part abutment-screw-container absolute left-[3.50rem] top-[1.5rem]">
            {shouldShowHighlight("screw") && (
              <svg className="highlight-overlay absolute pointer-events-none left-[-1.25rem] top-[-1.25rem] overflow-visible" width="98" height="139">
                <ellipse cx="49" cy="75.5" rx="69" ry="61" fill="#64c8ff" fillOpacity={0.65} />
              </svg>
            )}
            <img src={abutmentScrew} alt="Abutment Screw" className={`leg-part-image relative transition-all ${fade("screw")} ${highlight("screw")}`} style={{ width: '3.60394rem', height: ' 6.17075rem' }} draggable={false} />
          </div>

          {/* Socket */}
          <div className="leg-part socket-container absolute left-[5rem] top-[7.5rem]">
            {shouldShowHighlight("socket") && (
              <svg className="highlight-overlay absolute pointer-events-none left-[-1.5rem] top-[-1.25rem] overflow-visible" width="100" height="142">
                <ellipse cx="50" cy="75" rx="58" ry="59" fill="#64c8ff" fillOpacity={0.65} />
              </svg>
            )}
            <img src={Socket} alt="Socket" className={`leg-part-image relative transition-all ${fade("socket")} ${highlight("socket")}`} style={{ width: '3.11269rem', height: '6.39394rem' }} draggable={false} />
          </div>
          ...
          <div className="leg-part calf-container absolute left-[5.8rem] top-[15rem]">
            {shouldShowHighlight("calf") && (
              <svg className="highlight-overlay absolute pointer-events-none left-[-1.25rem] top-[-1.25rem] overflow-visible" width="140" height="180" viewBox="0 0 140 180">
                <ellipse cx="40.5" cy="55" rx="65" ry="55" fill="#64c8ff" fillOpacity={0.70} />
              </svg>
            )}
            <img src={Calf} alt="Calf" className={`leg-part-image relative transition-all ${fade("calf")} ${highlight("calf")}`} style={{ width: '2.04694rem', height: '4.02338rem' }} draggable={false} />
          </div>

          {/* Pylon */}
          <div className="leg-part pylon-container absolute left-[6.5rem] top-[19rem]">
            {shouldShowHighlight("pylon") && (
              <svg className="highlight-overlay absolute pointer-events-none left-[-1.25rem] top-[-1.25rem] overflow-visible" width="51" height="91">
                <ellipse cx="25.5" cy="58.5" rx="60" ry="50" fill="#64c8ff" fillOpacity={0.70} />
              </svg>
            )}
            <img src={Pylon} alt="Pylon" className={`leg-part-image relative transition-all ${fade("pylon")} ${highlight("pylon")}`} style={{ width: '0.6885rem', height: '3.20113rem' }} draggable={false} />
          </div>

          {/* Ankle */}
          <div className="leg-part ankle-container absolute left-[6.45rem] top-[22.25rem]">
            {shouldShowHighlight("ankle") && (
              <svg className="highlight-overlay absolute pointer-events-none left-[-1.25rem] top-[-1.25rem] overflow-visible" width="52" height="53">
                <ellipse cx="30" cy="25" rx="40" ry="35" fill="#64c8ff" fillOpacity={0.65} />
              </svg>
            )}
            <img src={Ankle} alt="Ankle" className={`leg-part-image relative transition-all ${fade("ankle")} ${highlight("ankle")}`} style={{ width: '0.76731rem', height: '0.82775rem' }} draggable={false} />
          </div>
